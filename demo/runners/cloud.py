...
async def main(args):
    cloud_agent = await create_agent_with_args(args, ident="cloud")

    try:
        log_status(
            "#7 Provision an agent and wallet, get back configuration details"
            + (
                f" (Wallet type: {cloud_agent.wallet_type})"
                if cloud_agent.wallet_type
                else ""
            )
        )
        agent = CloudAgent(
            "cloud.agent",
            cloud_agent.start_port,
            cloud_agent.start_port + 1,
            genesis_data=cloud_agent.genesis_txns,
            no_auto=cloud_agent.no_auto,
            timing=cloud_agent.show_timing,
            multitenant=cloud_agent.multitenant,
            mediation=cloud_agent.mediation,
            wallet_type=cloud_agent.wallet_type,
        )

        await cloud_agent.initialize(the_agent=agent)

        log_status("#9 Input invitation details")
        await input_invitation(cloud_agent)

        options = "    (3) Send Message\n" "    (4) Input New Invitation\n"
        if cloud_agent.multitenant:
            options += "    (W) Create and/or Enable Wallet\n"
        options += "    (X) Exit?\n[3/4/{}X] ".format(
            "W/" if cloud_agent.multitenant else "",
        )
        async for option in prompt_loop(options):
            ...
            if option is None or option in "xX":
            ...
            elif option in "wW" and cloud_agent.multitenant:
                target_wallet_name = await prompt("Enter wallet name: ")
                include_subwallet_webhook = await prompt(
                    "(Y/N) Create sub-wallet webhook target: "
                )
                if include_subwallet_webhook.lower() == "y":
                    await cloud_agent.agent.register_or_switch_wallet(
                        target_wallet_name,
                        webhook_port=cloud_agent.agent.get_new_webhook_port(),
                        mediator_agent=cloud_agent.mediator_agent,
                    )
                else:
                    await cloud_agent.agent.register_or_switch_wallet(
                        target_wallet_name,
                ...
                if msg:
                    await cloud_agent.agent.admin_POST(
        ...
        if cloud_agent.show_timing:
            timing = await cloud_agent.agent.fetch_timing()
            if timing:
                for line in cloud_agent.agent.format_timing(timing):
                    log_msg(line)

    finally:
        terminated = await cloud_agent.terminate()
...
if __name__ == "__main__":
    parser = arg_parser(ident="cloud", port=8090)
